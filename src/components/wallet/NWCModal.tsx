// Ported from v1 src/components/NWCModal.tsx
// Stripped: Supabase URI storage (no DB write of NWC URI)
// v2: URI stored in OPFS Vault — see comment below
// No other decontamination required (modal was clean)

/**
 * NWC Connection Modal
 *
 * Collects and validates a Nostr Wallet Connect (NWC) URI from the user.
 * v2: URI is NOT stored in Supabase. The caller is responsible for
 * writing the validated URI to OPFS Vault via the vault API:
 *   await opfsVault.set("nwc_uri", nwcUri);
 *
 * This satisfies Axiom 3 (no secrets in managed SaaS databases) and
 * security invariant S3 (no secret in Supabase).
 */
...
interface NWCModalProps {
  isOpen: boolean;
  onClose: () => void;
  /** Called with the validated URI — caller must write to OPFS Vault */
...
const NWCModal: React.FC<NWCModalProps> = ({
  ...
  const validateNWCUri = (uri: string): ValidationResult => {
    ...
    try {
      ...
      if (!relay) {
        ...
        warnings.push(
          "Consider using secure WebSocket (wss://) for better security"
        );
      }
      ...
      if (!permissions) {
        warnings.push(
          "No permissions specified — wallet may have limited functionality"
        );
      }
    } catch {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validation.isValid && !isLoading) {
      // v2: URI stored in OPFS Vault — caller handles vault write
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">
            Enter your Nostr Wallet Connect (NWC) URI. It will be stored
            securely in your OPFS Vault on this device.
          </p>
        </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <ul className="text-sm text-blue-800 space-y-1">
                <li>
                  • <strong>Alby:</strong> Settings → Wallet Connect
                </li>
                <li>
                  • <strong>Zeus:</strong> Settings → NWC
                </li>
                <li>
                  • <strong>Mutiny:</strong> Settings → Nostr Wallet Connect
                </li>
                <li>
                  • <strong>Other wallets:</strong> Look for "NWC" or "Wallet
                  Connect"
                </li>
              </ul>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  Your NWC URI contains sensitive information. It is encrypted
                  and stored only in your OPFS Vault on this device — never
                  sent to any server.
                </p>
              </div>
            </div>
